...
if(mdl_chs$data_lvl == "aggipd") {
  chsn_net <- combine_network(
    ...
    armpart,
    cntrstpart)
  mycor <- matrix(rep(0, 1), nrow = 1)
  diag(mycor) <- 1
  chsn_net <- add_integration(chsn_net,
                              value_1 = distr(qnorm, value_1, value_1_sd),
                              n_int = 1L, cor = mycor)
...
if(testrun) {
  mdl <- "somestring" } else {
    mdl <- nma(chsn_net,
               ...
               prior_reg = normal(scale = 10), chains = 4, cores = 4, iter = 2000)
  }


filename <- paste0("hba1c_no_inter_m", paste(mdl_chs, collapse = "_"))
if(dir.exists("Scratch_data/")) {
